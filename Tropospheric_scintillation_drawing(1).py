...
def calculate_L(hl, degree):
    degree = degree * math.pi / 180  # convert angle to radian
    x1 = 2 * hl
...
def calculate_standard_deviation_for_path(standard_deviation_for_signal, f, g, degree):
    ...
    return standard_deviation_for_signal * f ** (7 / 12) * g / (math.sin(degree)) ** 1.2
...
def calculateA_242(Kw, f, degree, percentage_p):
    degree = degree * math.pi / 180 * 1e+3  # convert from degree to mrad
    x1 = 10 * math.log10(Kw)
    x2 = 9 * math.log10(f)
    # x5 = -1.8 - 5.6 * math.log10((1.1 + abs(cos(2*26*math.pi/180)) ** 0.7))
    x3 = 55 * math.log10(1 + degree)  # 单位是mrad
    x4 = 10 * math.log(percentage_p)
    # A_242 = x1 + x2 - x3 - x4 -x5
    A_242 = x1 + x2 - x3 - x4
    return A_242
...
def calculate_deputy_A(e, elevation):
    return -1 * (55 / (1 + elevation)) * math.log10(math.e)


def calculate_deputy_A2(A2, x, Deff, f, hl, degree):
    degree = degree * math.pi / 180 * 10 ** 3  # mrad
...
def calculate_Long_term_fading(e, p, q, table_data_waterVapour, f, hl, percentage_p, degree):
    # 计算2.4.1
...
def drawing(A_241_list, A_242_list, A_243_list, x, degree_interval):
    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False
    plt.xticks(np.arange(0.5,9,0.5))
    plt.yticks(np.arange(0,90,10))
    plt.xlabel("自由空间仰度")
    plt.ylabel("闪烁衰落(dB)")
    plt.grid(linestyle = '-', which= 'major')
    plt.grid(linestyle = '-', which= 'minor', alpha = 0.1)
    plt.plot(x, A_241_list, color='g', linestyle='-', linewidth='0.7', label='2.4.1')
    plt.plot(x, A_242_list, color='r', linestyle='-', linewidth='0.7', label='2.4.2')
    plt.plot(degree_interval, A_243_list, color='b', linestyle='-', linewidth='0.7', label='2.4.3')
    plt.legend()
    plt.show()
...
if __name__ == '__main__':
    ...
    A_242_list = []
    A_243_list = []
    ax_x = np.arange(0.5, 8.1, 0.1)  # 角度范围
    for degree in ax_x:
    ...
    A2 = calculate_Long_term_fading(e, p, q, table_data_waterVapour, f, hl, percentage_p, degree=5)
    x = calculate_x(Deff, f, L)
    deputy_A2 = calculate_deputy_A2(A2, x, Deff, f, hl, degree=5)
    elevation2 = calculate_elevation(A2, Kw, f, percentage_p)
    degree_interpolation = np.arange(elevation1, elevation2, 1)
    # convert mrad to degree
    degree_interval= np.degrees(degree_interpolation/1000)
    for degree_243 in degree_interpolation:
        A_243 = calculate_Flickering_fading(A1, deputy_A1, A2, deputy_A2, elevation1, elevation2, degree_243)
        # print(A_243)
        A_243_list.append(A_243)
    # degree_interval = [i+2 for i in degree_interval]
    drawing(A_241_list, A_242_list, A_243_list, ax_x, degree_interval)
